...
function [NLML,D_NLML,K]=likelihood(hyp)
...
%Varianza dei dati di alta a bassa qualita
...
[N_H,D] = size(X_H);
N = N_L+N_H;
% Covariance matrix of low fidelity
K_LL = k(X_L, X_L, hyp(1:D+1),0);
% Convert variance matrix to sparse format
% Covariance matrix of Mix fidelity
K_LH = rho*k(X_L, X_H, hyp(1:D+1),0);
K_HL = rho*k(X_H, X_L, hyp(1:D+1),0);
% Covariance matrix of High fidelity
...
% alpha e' y'K^{-1}y tramite cholesky
% Solve the lower triangular system L * x = y to obtain the solution vector
% x: (L\y)
alpha = L'\(L\y);
